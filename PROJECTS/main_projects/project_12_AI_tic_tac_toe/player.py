...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            #  we are going to check that this is a correct value by trying to cast it to an integer,
            # and if it's not, then we say its invalid, if that spot is not available on the board, we also say it's invalid
            try:
                val = int(square)
...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        if state.current_winner == other_player:
            ...
            return {"position": None, "score": 0}      # no empty squares left, it's a tie
        ...
        for possible_move in state.available_moves():
            #  step 1: make a new move, try that spot
